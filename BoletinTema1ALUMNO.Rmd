...
date:  "15/02/2023"  #Pondría la fecha del día actual
params:
  lang: ES
...
#Sí presentan el mismo comportamiento, pero no cimplen la distibución Gaussiana
```
...
```{r}
is.na(cabbages)#Como todo es false, no hay valores faltantes
#Podemos observar que es una matriz de 60 columnas y 4 filas en las que cada 10 valores cambian las dos primeras filas mientras que las otras 2 son siempre independientes

is.na(Chile)#Sí que hay valores perdidos ya que se observan varios trues
#Es una matriz de 2700 columnas y 8 filas en las que en orden de izquierda a derecha representan, región, población,sexo,edad,educación,ingreso, statusquo y si ha votado o no.
...
# Planteamos 2 modelos lineales, uno para los datos de cada factor
Model1 <- lm(cabbages$VitC ~ cabbages$HeadWt, data = cabbages)
Model2 <- lm(cabbages$VitC ~ cabbages$HeadWt, data = cabbages)
# Representamos las rectas correspondientes a cada modelo lineal
abline(Model1, lty=2)
abline(Model2)
```
